...
Début
    ...
    Ecrire (phrase);
    // Décryptage d'une phrase
    n <- strlen (phrase);
    k <- 0;
    Pour (i<-0;i<n;i++) 
        Si (phrase[i]='I' ET phrase[i+1]='T') Alors 
            k <- k + 1; // Compter le nombre d'apparition de la suite 'IJ' dans la phrase
        FinSi
    FinPour
    Pour (i<-0;i<n;i++) 
        Si (phrase[i]='I' ET phrase[i+1]='T') Alors 
            j <- i;
            TantQue ( j< n &ET phrase[j] <> '\0') 
                l <- phrase[j];
                phrase[j] <- phrase[j+1]; // Permitation de 'I'
                phrase[j+1] <- l;
                j <- j + !;
            FinTantQue
        Pour (k<-0;k<6;k++) 
            Si (phrase[i]='T' ET phrase[i+1]=voyelle[k]) Alors 
                Pour (j = i; phrase[j] <>'\0') 
                    l <- phrase[j];
                    phrase[j]<- phrase[j+1]; // Permitation de 'T'
                    phrase[j<-1] <- l;
...
int main ()
{
    ...
    for (i=0;i<n;i++)
    {
        if (phrase[i]=='I' && phrase[i+1]=='T')
        {
            k++; // Compter le nombre d'apparition de la suite 'IJ' dans la phrase
        }
    }
    for (i=0;i<n;i++)
    {
        ...
        for (k=0;k<6;k++)
        {
            if (phrase[i]=='T' && phrase[i+1]==voyelle[k])
            {
                j = i;
                while ( j< n && phrase[j] != '\0')
                {
